/* ===========================================================================
   ...
   สารบัญ:
     ...
     5. หน้ารายละเอียดรถ
     6. หน้าประกันรถมอไซ
     7. หน้าแพ็คเกจจิ้ง (วัสดุ/อุปกรณ์แพ็คของส่งพัสดุ)
     8. ฟอร์ม เพิ่ม/แก้ไข/อัปโหลดไฟล์
   =========================================================================== */
...
var ROUTES = {
  ...
  packaging: { title: '📦 วัสดุแพ็คของ', render: renderPackagingPage },
  packagingDetail: { title: 'รายละเอียดรายการแพ็คเกจจิ้ง', render: renderPackagingDetailPage },
...
function setActiveNav(route) {
  ...
  items.forEach(function (el) {
    el.classList.toggle('active', el.getAttribute('data-route') === route ||
      ...
      (route === 'packagingDetail' && el.getAttribute('data-route') === 'packaging'));
  });
}
...
/* --------------------------------------------------------------------
   7) หน้าแพ็คเกจจิ้ง (วัสดุ/อุปกรณ์แพ็คของส่งพัสดุ)
   เน้นดูเป็น "ประวัติของรายการ" (เช่น กล่องพัสดุเบอร์ 2) มากกว่า "ประวัติของบิล" — คลิกเข้ารายการ
   จะเห็นประวัติการสั่งซื้อทุกครั้งของรายการนั้นย้อนหลัง พร้อมราคา/จำนวน/เลขบิล/ไฟล์บิลแนบ
   -------------------------------------------------------------------- */

function renderPackagingPage(container) {
  container.innerHTML =
    '<div class="toolbar"><div></div><div style="display:flex;gap:10px;">' +
    '<button class="btn secondary" id="btn-add-pkg-order">➕ บันทึกการสั่งซื้อ</button>' +
    '<button class="btn" id="btn-add-pkg-item">➕ เพิ่มรายการใหม่</button>' +
    '</div></div>' +
    '<div id="pkg-grid" class="pkg-grid"><div class="empty-state">กำลังโหลดข้อมูล...</div></div>';

  document.getElementById('btn-add-pkg-item').addEventListener('click', function () {
    openPackagingItemFormModal(null, function () { renderPackagingPage(container); });
  });
  document.getElementById('btn-add-pkg-order').addEventListener('click', function () {
    openPackagingOrderPickerModal(function () { renderPackagingPage(container); });
  });

  apiGet('listPackagingItems').then(function (items) {
    var grid = document.getElementById('pkg-grid');
    if (!items.length) {
      grid.innerHTML = '<div class="empty-state">ยังไม่มีรายการแพ็คเกจจิ้ง กด "เพิ่มรายการใหม่" เพื่อเริ่มต้น</div>';
      return;
    }
    grid.innerHTML = items.map(renderPackagingItemCard_).join('');
    items.forEach(function (it) {
      var el = document.getElementById('pkg-card-' + it.itemId);
      if (el) el.addEventListener('click', function (e) {
        if (e.target.closest('button') || e.target.closest('a')) return;
        navigate('packagingDetail', { itemId: it.itemId });
      });
    });
  }).catch(function () {
    document.getElementById('pkg-grid').innerHTML = '<div class="empty-state">ไม่สามารถโหลดข้อมูลได้</div>';
  });
}

function renderPackagingItemCard_(it) {
  var photo = it.referenceImageUrl
    ? '<img src="' + esc(it.referenceImageUrl) + '" alt="' + esc(it.itemName) + '">'
    : '<div class="no-photo">📦</div>';
  var lastSummary = it.orderCount
    ? '<div class="kv-grid">' +
      kvInline_('สั่งล่าสุด', fmtDate(it.lastOrderDate)) +
      kvInline_('จำนวน', fmtNumber(it.lastQuantity) + (it.unit ? ' ' + it.unit : '')) +
      kvInline_('ราคา', fmtCurrency(it.lastUnitPrice || it.lastTotalCost)) +
      '</div>' +
      (it.lastBillFileUrl ? '<div class="file-links"><a href="' + esc(it.lastBillFileUrl) + '" target="_blank" rel="noopener">📎 ดูบิลล่าสุด</a></div>' : '')
    : '<div class="empty-state" style="padding:8px 0;">ยังไม่เคยสั่งซื้อ</div>';

  return '<div class="pkg-card" id="pkg-card-' + it.itemId + '">' +
    '<div class="pkg-photo">' + photo + '</div>' +
    '<div class="pkg-body">' +
    '<div class="pkg-title">' + esc(it.itemName) + '</div>' +
    '<div class="pkg-meta">' + esc(it.category || 'ไม่ระบุประเภท') + (it.unit ? ' &nbsp;|&nbsp; หน่วย: ' + esc(it.unit) : '') + '</div>' +
    lastSummary +
    '<div class="card-footer">' +
    '<button class="btn secondary small" onclick="event.stopPropagation();openPackagingOrderModal(' + JSON.stringify({ itemId: it.itemId, itemName: it.itemName, unit: it.unit }).replace(/"/g, '&quot;') + ',null,function(){navigate(\'packaging\');});">➕ สั่งซื้อ</button>' +
    '<button class="btn ghost small" onclick="event.stopPropagation();navigate(\'packagingDetail\',{itemId:\'' + it.itemId + '\'});">ดูประวัติ</button>' +
    '</div>' +
    '</div>' +
    '</div>';
}

/* ---------------- รายละเอียดรายการแพ็คเกจจิ้ง (ประวัติการสั่งซื้อ) ---------------- */

function renderPackagingDetailPage(container, params) {
  var itemId = params.itemId;
  container.innerHTML = '<div class="empty-state">กำลังโหลดข้อมูล...</div>';

  apiGet('getPackagingItemDetail', { itemId: itemId }).then(function (data) {
    renderPackagingDetailBody_(container, itemId, data);
  }).catch(function () {
    container.innerHTML = '<div class="empty-state">ไม่พบข้อมูลรายการ</div>';
  });
}

function renderPackagingDetailBody_(container, itemId, data) {
  var item = data.item, orders = data.orders;
  var photo = item.referenceImageUrl
    ? '<img src="' + esc(item.referenceImageUrl) + '" alt="' + esc(item.itemName) + '">'
    : '<div class="no-photo">📦</div>';

  var reload = function () {
    apiGet('getPackagingItemDetail', { itemId: itemId }).then(function (d) {
      renderPackagingDetailBody_(container, itemId, d);
    }).catch(function () {});
  };

  var orderRows = orders.map(function (o) {
    return '<div class="record-card"><div class="rc-head">' +
      '<span class="rc-title">' + fmtDate(o.orderDate) + ' &nbsp;—&nbsp; ' + fmtNumber(o.quantity) + ' ' + esc(item.unit || '') + '</span>' +
      '<span style="font-weight:700;">' + fmtCurrency(o.totalCost || o.unitPrice) + '</span></div>' +
      '<div class="kv-grid">' +
      kvInline_('ราคาต่อหน่วย', fmtCurrency(o.unitPrice)) +
      kvInline_('เลขที่บิล', o.billNumber) +
      kvInline_('ร้าน/ผู้ขาย', o.supplier) +
      '</div>' +
      (o.notes ? '<div class="kv-grid">' + kvInline_('หมายเหตุ', o.notes) + '</div>' : '') +
      '<div class="file-links">' + fileLinkOrUpload_(o.billFileUrl, 'ดูไฟล์บิล', 'uploadPackagingBillFile', { orderId: o.orderId }) +
      '<a href="#" onclick="openPackagingOrderModal(' + JSON.stringify({ itemId: item.itemId, itemName: item.itemName, unit: item.unit }).replace(/"/g, '&quot;') + ',' + JSON.stringify(o).replace(/"/g, '&quot;') + ');return false;">✏️ แก้ไข</a>' +
      '<a href="#" style="color:#d93025;" onclick="deletePackagingOrderRecord_(' + JSON.stringify(o.orderId) + ');return false;">🗑️ ลบ</a>' +
      '</div></div>';
  }).join('') || '<div class="empty-state">ยังไม่เคยสั่งซื้อรายการนี้</div>';

  container.innerHTML =
    '<button class="btn ghost small" id="btn-back-pkg">← กลับหน้ารายการทั้งหมด</button>' +
    '<div class="panel" style="display:flex;gap:18px;flex-wrap:wrap;margin-top:12px;">' +
    '<div class="pkg-detail-photo">' + photo + '</div>' +
    '<div style="flex:1;min-width:220px;">' +
    '<div style="font-size:20px;font-weight:800;">📦 ' + esc(item.itemName) + '</div>' +
    '<div style="color:var(--text-soft);margin:4px 0 10px;">' + esc(item.category || 'ไม่ระบุประเภท') + (item.unit ? ' &nbsp;|&nbsp; หน่วยนับ: ' + esc(item.unit) : '') + '</div>' +
    (item.notes ? '<div style="margin-bottom:10px;">' + esc(item.notes) + '</div>' : '') +
    '<div style="display:flex;gap:10px;flex-wrap:wrap;">' +
    '<button class="btn small" id="btn-add-pkg-order-detail">➕ บันทึกการสั่งซื้อ</button>' +
    '<button class="btn secondary small" id="btn-edit-pkg-item">✏️ แก้ไขรายการ</button>' +
    fileLinkUpload_('เปลี่ยนรูปตัวอย่าง', 'uploadPackagingItemImage', { itemId: item.itemId }) +
    '</div>' +
    '</div>' +
    '</div>' +
    '<h3 style="margin:18px 0 10px;">ประวัติการสั่งซื้อ (' + orders.length + ' ครั้ง)</h3>' +
    orderRows;

  document.getElementById('btn-back-pkg').addEventListener('click', function () { navigate('packaging'); });
  document.getElementById('btn-add-pkg-order-detail').addEventListener('click', function () {
    openPackagingOrderModal({ itemId: item.itemId, itemName: item.itemName, unit: item.unit }, null, reload);
  });
  document.getElementById('btn-edit-pkg-item').addEventListener('click', function () {
    openPackagingItemFormModal(item, reload);
  });
}

/** ลบประวัติการสั่งซื้อ — ยืนยันก่อนเสมอเพื่อกันลบพลาด (การลบเป็นการลบจริง กู้คืนไม่ได้) */
function deletePackagingOrderRecord_(orderId) {
  if (!window.confirm('ยืนยันลบประวัติการสั่งซื้อนี้? การลบไม่สามารถกู้คืนได้')) return;
  apiPost('deletePackagingOrder', { orderId: orderId }).then(function () {
    showToast('ลบประวัติเรียบร้อยแล้ว', 'success');
    navigate('packagingDetail', APP_STATE.params);
  }).catch(function () {});
}
...
/** ฟอร์มเพิ่ม/แก้ไข "รายการ" แพ็คเกจจิ้ง (ตัวสินค้า/สเปก ไม่ใช่ประวัติการสั่งซื้อ) */
function openPackagingItemFormModal(item, onDone) {
  getFormOptionsCached().then(function (opts) {
    var isEdit = !!item;
    var r = item || {};
    var body = '<div class="form-grid">' +
      field_({ name: 'itemName', label: 'ชื่อรายการ', value: r.itemName, full: true, hint: 'เช่น กล่องพัสดุเบอร์ 2, เทปใส 2 นิ้ว, ถุงไปรษณีย์ไซส์ M' }) +
      field_({ name: 'category', label: 'ประเภท', type: 'select', value: r.category, options: opts.packagingCategories }) +
      field_({ name: 'unit', label: 'หน่วยนับ', type: 'select', value: r.unit, options: opts.packagingUnits }) +
      (isEdit ? field_({ name: 'status', label: 'สถานะ', type: 'select', value: r.status || 'ใช้งาน', options: ['ใช้งาน', 'ปิดใช้งาน'] }) : '') +
      field_({ name: 'notes', label: 'หมายเหตุ', type: 'textarea', value: r.notes, full: true }) +
      '</div>';

    openModal({
      title: isEdit ? '✏️ แก้ไขรายการแพ็คเกจจิ้ง' : '➕ เพิ่มรายการแพ็คเกจจิ้งใหม่',
      bodyHtml: body,
      onSave: function (values) {
        if (isEdit) return apiPost('updatePackagingItem', { itemId: r.itemId, data: values });
        return apiPost('createPackagingItem', values);
      },
      onDone: onDone
    });
  });
...
/**
 * ฟอร์มบันทึกการสั่งซื้อ — ใช้ตอนที่รู้ itemId อยู่แล้ว (กดจากการ์ดรายการ หรือจากหน้ารายละเอียด)
 * item = { itemId, itemName, unit }, order = record เดิมถ้าเป็นการแก้ไข (null ถ้าเพิ่มใหม่)
 */
function openPackagingOrderModal(item, order, onDone) {
  var isEdit = !!order;
  var r = order || {};
  var body = '<div class="form-grid">' +
    field_({ name: 'orderDate', label: 'วันที่สั่งซื้อ', type: 'date', value: r.orderDate }) +
    field_({ name: 'quantity', label: 'จำนวน' + (item.unit ? ' (' + item.unit + ')' : ''), type: 'number', value: r.quantity }) +
    field_({ name: 'unitPrice', label: 'ราคาต่อหน่วย (บาท)', type: 'number', value: r.unitPrice }) +
    field_({ name: 'totalCost', label: 'ราคารวมทั้งหมด (บาท)', type: 'number', value: r.totalCost }) +
    field_({ name: 'billNumber', label: 'เลขที่บิล/ใบเสร็จ', value: r.billNumber }) +
    field_({ name: 'supplier', label: 'ร้าน/ผู้ขาย', value: r.supplier }) +
    field_({ name: 'notes', label: 'หมายเหตุ', type: 'textarea', value: r.notes, full: true }) +
    '</div>';

  openModal({
    title: (isEdit ? '✏️ แก้ไขการสั่งซื้อ: ' : '➕ บันทึกการสั่งซื้อ: ') + item.itemName,
    bodyHtml: body,
    onSave: function (values) {
      if (isEdit) return apiPost('updatePackagingOrder', { orderId: r.orderId, data: values });
      values.itemId = item.itemId;
      return apiPost('createPackagingOrder', values);
    },
    onDone: onDone
  });
}
...
/**
 * ฟอร์มบันทึกการสั่งซื้อแบบ "เลือกรายการเอง" (ปุ่มบนสุดของหน้าแพ็คเกจจิ้ง) — เลือกจากรายการที่มีอยู่
 * ผ่านดรอปดาวน์ หรือเลือก "➕ เพิ่มรายการใหม่..." เพื่อพิมพ์ชื่อรายการใหม่แล้วสร้างพร้อมกันในขั้นตอนเดียว
 */
function openPackagingOrderPickerModal(onDone) {
  Promise.all([apiGet('listPackagingItems'), getFormOptionsCached()]).then(function (results) {
    var items = results[0], opts = results[1];
    var itemOptionsHtml = items.map(function (it) {
      return '<option value="' + esc(it.itemId) + '">' + esc(it.itemName) + (it.unit ? ' (' + esc(it.unit) + ')' : '') + '</option>';
    }).join('');

    var body =
      '<div class="form-grid">' +
      '<div class="form-field full"><label>เลือกรายการ</label>' +
      '<select id="pkg-item-select">' +
      '<option value="">-- เลือกรายการที่เคยสั่ง --</option>' +
      itemOptionsHtml +
      '<option value="__new__">➕ เพิ่มรายการใหม่...</option>' +
      '</select></div>' +
      '</div>' +
      '<div id="pkg-new-item-fields" class="form-grid" style="display:none;margin-top:4px;">' +
      field_({ name: 'newItemName', label: 'ชื่อรายการใหม่', full: true, hint: 'เช่น กล่องพัสดุเบอร์ 2, เทปใส 2 นิ้ว' }) +
      field_({ name: 'newItemCategory', label: 'ประเภท', type: 'select', options: opts.packagingCategories }) +
      field_({ name: 'newItemUnit', label: 'หน่วยนับ', type: 'select', options: opts.packagingUnits }) +
      '</div>' +
      '<div class="form-grid" style="margin-top:14px;">' +
      field_({ name: 'orderDate', label: 'วันที่สั่งซื้อ', type: 'date' }) +
      field_({ name: 'quantity', label: 'จำนวน', type: 'number' }) +
      field_({ name: 'unitPrice', label: 'ราคาต่อหน่วย (บาท)', type: 'number' }) +
      field_({ name: 'totalCost', label: 'ราคารวมทั้งหมด (บาท)', type: 'number' }) +
      field_({ name: 'billNumber', label: 'เลขที่บิล/ใบเสร็จ' }) +
      field_({ name: 'supplier', label: 'ร้าน/ผู้ขาย' }) +
      field_({ name: 'notes', label: 'หมายเหตุ', type: 'textarea', full: true }) +
      '</div>';

    openModal({
      title: '➕ บันทึกการสั่งซื้อแพ็คเกจจิ้ง',
      bodyHtml: body,
      onSave: function (values) {
        var sel = document.getElementById('pkg-item-select').value;
        if (!sel) return Promise.reject(new Error('กรุณาเลือกรายการก่อนบันทึก'));
        if (sel === '__new__') {
          if (!values.newItemName) return Promise.reject(new Error('กรุณากรอกชื่อรายการใหม่'));
          return apiPost('createPackagingItem', {
            itemName: values.newItemName, category: values.newItemCategory, unit: values.newItemUnit
          }).then(function (newItem) {
            values.itemId = newItem.itemId;
            return apiPost('createPackagingOrder', values);
          });
        }
        values.itemId = sel;
    ...
    var selectEl = document.getElementById('pkg-item-select');
    var newFieldsEl = document.getElementById('pkg-new-item-fields');
    if (selectEl) selectEl.addEventListener('change', function () {
      newFieldsEl.style.display = selectEl.value === '__new__' ? '' : 'none';
    });
  }).catch(function () {});
}
