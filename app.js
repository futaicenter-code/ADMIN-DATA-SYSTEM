...
function apiPostRaw_(action, payload) {
  ...
  return fetch(CONFIG.API_URL, {
    ...
    body: JSON.stringify({ action: action, token: CONFIG.APP_TOKEN || '', payload: payload || {} })
...
function renderExpensesTab_(d) {
  var rows = d.expenses.map(function (r) {
    return '<div class="record-card"><div class="rc-head"><span class="rc-title">' + esc(r.category || 'ค่าใช้จ่าย') + '</span>' +
      ...
      '<div class="file-links">' + fileLinkOrUpload_(r.fileUrl, 'ดูไฟล์แนบ', 'uploadExpenseFile', { expenseId: r.expenseId }) + '</div></div>';
  }).join('') || '<div class="empty-state">ยังไม่มีรายการค่าใช้จ่าย</div>';
  return '<div class="toolbar"><div></div><button class="btn small" id="btn-add-expense">➕ เพิ่มค่าใช้จ่าย</button></div>' + rows;
}
